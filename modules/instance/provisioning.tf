resource "null_resource" "docker-swarm-manager" {
  ...
  provisioner "file" {
    source      = "compose/docker-compose-v3.yml"
    destination = "/home/soul/docker-compose.yml"
  }
...
resource "null_resource" "docker-swarm-manager-join" {
  ...
  provisioner "local-exec" {
    command = "TOKEN=$(ssh -i /home/soul/.ssh/${var.ssh_credentials.private_key} -o StrictHostKeyChecking=no ${var.ssh_credentials.user}@${yandex_compute_instance.vm-manager[count.index].network_interface.0.nat_ip_address} docker swarm join-token -q worker); echo \"#!/usr/bin/env bash\nsudo docker swarm join --token $TOKEN ${yandex_compute_instance.vm-manager[count.index].network_interface.0.nat_ip_address}:2377\nexit 0\" >| join.sh"
  }
}
...
resource "null_resource" "docker-swarm-manager-start" {
  ...
  provisioner "remote-exec" {
    inline = [
      "docker stack deploy --compose-file /home/soul/docker-compose.yml sockshop-swarm"
    ]
  }
